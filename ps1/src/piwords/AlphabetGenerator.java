...
public class AlphabetGenerator {
    ...
    public static char[] generateFrequencyAlphabet(int base,
        ...
        Pattern notLetters = Pattern.compile("[^a-z]");
        for (int i = 0; i < trainingData.length; i++) {
            String lowerCaseTraining = trainingData[i].toLowerCase();
            String[] lowerCaseStrings = notLetters.split(lowerCaseTraining);
            for (int k = 0; k < lowerCaseStrings.length; k++) {
                for (int j = 0; j < lowerCaseStrings[k].length(); j++) {
                    characters.add(lowerCaseStrings[k].charAt(j));
                }
            }
        }

        Collections.sort(characters);
        double totalChars = (double) characters.size(); // get total characters in string
        char currentChar = characters.get(0); // get first character in string
        HashMap<Character, Integer> letterMap = new HashMap<Character, Integer>();
        for (int i = 0; i < characters.size(); i++) {
            char nextChar = characters.get(i); // get next character in string
            if (currentChar == nextChar) {
                continue;
            } else {
                letterMap.put(currentChar, i);
            }
            currentChar = nextChar;
        }
        letterMap.put(currentChar, characters.size());

        char[] lettersOut = new char[base];
        int startValue = 0;
        for (HashMap.Entry<Character, Integer> entry : letterMap.entrySet()) {
            char thisChar = entry.getKey();
            int thisValue = entry.getValue();
            double base26 = base * thisValue / totalChars;
            for (int i = startValue; i < base26; i++) {
                lettersOut[i] = thisChar;
                startValue++;
            }
        }
        return (lettersOut);
...
//        AlphabetGenerator.generateFrequencyAlphabet(100, WORD_LIST);
//    }
//}
